...
public final class TeamInviteSubcommand extends SubCommandHandler {
    ...
    public void execute(@NotNull Player player, @NonNull String[] arguments) {
        if (arguments.length == 0) {
            RavenMedia.sendMessage(player, LanguagePath.MESSAGE_COMMAND_TEAM_INVITATION_USAGE_INVITE);
            return;
        }
        ...
        switch (this.teamInvitationManager.invitePlayer(player.getUniqueId(), targetPlayerName)) {
            ...
            case TARGET_IS_IN_TEAM ->
                    RavenMedia.sendMessage(player, LanguagePath.MESSAGE_COMMAND_TEAM_GENERAL_ERROR_TARGET_IS_IN_TEAM);
            case TARGET_IS_IN_ANOTHER_TEAM ->
                    RavenMedia.sendMessage(player, LanguagePath.MESSAGE_COMMAND_TEAM_GENERAL_ERROR_TARGET_IS_IN_ANOTHER_TEAM);
            case TARGET_IS_SELF ->
                    RavenMedia.sendMessage(player, LanguagePath.MESSAGE_COMMAND_TEAM_GENERAL_ERROR_TARGET_IS_SELF);
            case TARGET_ALREADY_INVITED ->
                    RavenMedia.sendMessage(player, LanguagePath.MESSAGE_COMMAND_TEAM_INVITATION_DECLINED);
        }
    }
