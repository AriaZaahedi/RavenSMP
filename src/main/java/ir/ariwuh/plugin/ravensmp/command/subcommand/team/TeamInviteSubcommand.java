...
public final class TeamInviteSubcommand extends RavenSubCommandHandler {
    ...
    public void execute(@NotNull Player player, @NonNull String[] arguments) {
        if (arguments.length == 0) {
            RavenMedia.sendMessage(player, MESSAGE_COMMAND_TEAM_INVITATION_USAGE_INVITE);
            return;
        }
        ...
        switch (this.teamInvitationManager.invitePlayer(player.getUniqueId(), targetPlayerName)) {
            case PLAYER_LACKING_TEAM ->
                    RavenMedia.sendMessage(player, MESSAGE_COMMAND_TEAM_GENERAL_ERROR_PLAYER_LACKING_TEAM);
            case PLAYER_NOT_LEADER ->
                    RavenMedia.sendMessage(player, MESSAGE_COMMAND_TEAM_GENERAL_ERROR_PLAYER_NOT_LEADER);
            case TEAM_ON_CAPACITY -> RavenMedia.sendMessage(player, MESSAGE_COMMAND_TEAM_INVITATION_ERROR_ON_CAPACITY);
            case TARGET_IS_SELF -> RavenMedia.sendMessage(player, MESSAGE_COMMAND_TEAM_GENERAL_ERROR_TARGET_IS_SELF);
            case TARGET_DISABLED_TEAM_INVITES ->
                    RavenMedia.sendMessage(player, MESSAGE_COMMAND_TEAM_INVITATION_ERROR_TARGET_INVITE_DISABLED);
            case TARGET_IS_IN_TEAM ->
                    RavenMedia.sendMessage(player, MESSAGE_COMMAND_TEAM_GENERAL_ERROR_TARGET_IS_IN_TEAM);
            case TARGET_IS_IN_ANOTHER_TEAM ->
                    RavenMedia.sendMessage(player, MESSAGE_COMMAND_TEAM_GENERAL_ERROR_TARGET_IS_IN_ANOTHER_TEAM);
            case TARGET_OFFLINE -> RavenMedia.sendMessage(player, MESSAGE_COMMAND_GENERAL_ERROR_PLAYER_OFFLINE);
            case TARGET_ALREADY_INVITED -> RavenMedia.sendMessage(player, MESSAGE_COMMAND_TEAM_INVITATION_DECLINED);
        }
    }
